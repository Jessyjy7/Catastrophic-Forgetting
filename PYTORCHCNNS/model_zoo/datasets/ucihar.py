...
def load_train_val_data(batch_size=64, train_val_split=0.9, shuffle=True, cuda=False):
    ...
    root_dir = os.path.join('datasets', 'ucihar')

    train_dataset = torchhd.datasets.UCIHAR(root=root_dir, train=True, download=True)
...
def load_test_data(batch_size=1000, shuffle=False, sampler=None, cuda=False):
    ...
    test_dataset = torchhd.datasets.UCIHAR(root=root_dir, train=False, download=True)
