...
def sequential_train_with_buffer_using_decoded(model, device, criterion, optimizer, epochs, test_loader):
    ...
    decoded_images = decoded_replay_buffer[1][:3]  
    for i, img in enumerate(decoded_images):
        # img = adjust_contrast(torch.from_numpy(img).reshape(1,1,img.shape[0], img.shape[1]), 1.05).reshape(img.shape[0], img.shape[1])
        plt.imshow(img, cmap="gray")
        plt.title(f"Decoded Image {i} (Digit {0})")
        plt.axis("off")
        plt.show()
...
# Sequential training without replay buffer
def lifelong_learning(model, device, criterion, optimizer, test_loader):
    print("\nStarting sequential training without replay buffer")
    for digit in range(10):
        print(f"\nTraining on digit {digit} only")
        digit_loader = get_digit_loader(digit, batch_size=args.batch_size, train=True)
        model.train()
        running_loss = 0.0
        for data, target in digit_loader:
            ...
            optimizer.step()
            running_loss += loss.item()
        print(f"Digit {digit}, Loss: {running_loss / len(digit_loader)}")
...
# Sequential training with incremental replay buffer (using original images for buffer)
def lifelong_learning_with_buffer(model, device, criterion, optimizer, test_loader):
    print("\nStarting sequential training with incremental replay buffer")
    replay_buffer = []

    for digit in range(10):
        print(f"\nTraining on digit {digit} with replay buffer")
        ...
        for seen_digit in range(digit):  
            ...
            seen_digit_labels = []
            for data, target in seen_digit_loader:
                seen_digit_data.extend(data[:5])
                seen_digit_labels.extend(target[:5])
                if len(seen_digit_data) >= 5:
                    seen_digit_data = seen_digit_data[:5]
                    seen_digit_labels = seen_digit_labels[:5]
                    break
            replay_buffer.extend(zip(seen_digit_data, seen_digit_labels))
        ...
        for data, target in digit_loader:
            current_digit_images.extend(data)
            current_digit_labels.extend(target)
        current_digit_images = current_digit_images  
        current_digit_labels = current_digit_labels

        # Combine replay buffer (decoded images) and current digit original data
        buffer_images = [torch.tensor(img, dtype=torch.float32).unsqueeze(0) if len(img.shape) == 3 else torch.tensor(img, dtype=torch.float32) for img, _ in replay_buffer]
        buffer_labels = [torch.tensor(label, dtype=torch.long) for _, label in replay_buffer]
        ...
        # Stack tensors to create a dataset
        # combined_dataset = TensorDataset(torch.cat(combined_images, dim=0), torch.stack(combined_labels)) #ORIGINAL
        combined_dataset = TensorDataset(torch.cat(current_digit_images, dim=0), torch.stack(current_digit_labels))
        combined_loader = DataLoader(combined_dataset, batch_size=args.batch_size, shuffle=True)
        ...
        for i, (data, target) in enumerate(combined_loader):
            ...
            if len(buffer_images)>0:
                ...
                train_data = torch.cat( (data, images), dim=0)
                train_labels = torch.cat( (target, labels), dim=0 )
                # print(train_data.shape, train_labels.shape)
                # input()
                data, target = train_data.to(device), train_labels.to(device)
            else:
                data, target = data.to(device), target.to(device)
            optimizer.zero_grad()
            output = model(data)
            ...
            optimizer.step()
            running_loss += loss.item()
        print(f"Digit {digit} -  Loss: {running_loss / len(combined_loader)}")
...
# Sequential training with incremental replay buffer (using decoded images)
def lifelong_learning_with_buffer_using_decoded(model, device, criterion, optimizer, test_loader):
    print("\nStarting sequential training with decoded replay buffer")
...
def train_with_decoded_buffer_only_incremental(model, device, criterion, optimizer, epochs, test_loader):
    ...
    # for digit in range(10):
    for digit in [1,0,2,3,4,5,6,7,8,9]:
...
# lifelong_learning(model, device, criterion, optimizer, test_loader)
# lifelong_learning_with_buffer(model, device, criterion, optimizer, test_loader)
# lifelong_learning_with_buffer_using_decoded(model, device, criterion, optimizer, test_loader)
